...
const ThemeToggle: React.FC = () => {
  ...
  return (
    <button
      onClick={toggleTheme}
      className="p-2.5 rounded-xl bg-bg-secondary border border-border-primary hover:border-indigo-500/30 transition-all duration-300 group relative overflow-hidden"
      aria-label="Toggle Theme"
    >
      <div className="relative w-5 h-5">
        {/* Sun Icon */}
        <div className={`absolute inset-0 transform transition-all duration-500 ${theme === 'dark' ? 'rotate-90 scale-0 opacity-0' : 'rotate-0 scale-100 opacity-100'}`}>
        ...
        {/* Moon Icon */}
        <div className={`absolute inset-0 transform transition-all duration-500 ${theme === 'light' ? '-rotate-90 scale-0 opacity-0' : 'rotate-0 scale-100 opacity-100'}`}>
